...
Download the `.apk` from the build URL (or expo.dev → CCC Agent → Builds). Sideload on Android. Agents sign in with the **HQ-issued agent code** (8 characters, hyphenated; 10-character codes already issued still work) at their polling unit. GPS must be within **100 ft** of the unit pin. Email/password is only a fallback. Do not put HQ Super Admin emails in the app.
