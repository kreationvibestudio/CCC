...
Download the `.apk` from the build URL (or expo.dev → CCC Agent → Builds). Sideload on Android. Agents sign in with the **HQ-issued agent code** (10 characters; older 8-character codes still work) at their polling unit. GPS must be within **100 ft** of the unit pin. Email/password is only a fallback. Do not put HQ Super Admin emails in the app.
