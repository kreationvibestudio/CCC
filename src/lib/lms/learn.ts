...
export async function loginVolunteerLearn(input: {
  ...
  if (phone.replace(/\D/g, "").length < 10) return { error: "Enter the phone number you registered with." };
...
async function requireLearner() {
  ...
  if (!session) return { error: "Please sign in to continue training." as const };
  return { volunteer: session.volunteer, campaign: session.campaign, admin: createServiceClient() };
}
...
export async function completeLearnModule(input: {
  ...
  const { data: module } = await admin.from("lms_modules").select("*").eq("id", input.moduleId).maybeSingle();
  if (!module) return { error: "Module not found." };
  const row = module as ModuleRow;
  ...
  await completeCourseIfReady(admin, {
    tenantId: volunteer.tenant_id,
    volunteerId: volunteer.id,
...
export async function submitLearnQuiz(input: { moduleId: string; answers: Record<string, number> }) {
  const gate = await requireLearner();
  if ("error" in gate) return { error: gate.error };
  const { volunteer, admin } = gate;
  const { data: module } = await admin.from("lms_modules").select("*").eq("id", input.moduleId).maybeSingle();
  if (!module) return { error: "Quiz not found." };
  const row = module as ModuleRow;
  const { data: course } = await admin.from("lms_courses").select("*").eq("id", row.course_id).maybeSingle();
  if (!course) return { error: "Course not found." };
  ...
  if (existing?.status === "completed") return { error: "You already passed this quiz." };
